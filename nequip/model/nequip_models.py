...
from nequip.nn import EwaldQeq, Qeq, SumEnergies
...
# === long-range electrostatics modules ===

@model_builder
def NequIPGNNElecEnergyModel(
    num_layers: int = 4,
    l_max: int = 1,
    parity: bool = True,
    num_features: int = 32,
    ...
    **kwargs,
    ...
    # === build model ===
    model = FullNequIPGNNElecEnergyModel(
        irreps_edge_sh=irreps_edge_sh,
        type_embed_num_features=num_features,
        feature_irreps_hidden=feature_irreps_hidden_list,
        radial_mlp_depth=radial_mlp_depth_list,
...
def FullNequIPGNNElecEnergyModel(
    r_max: float,
    pbc: bool,
    type_names: Sequence[str],
    # convnet params
    ...
    modules.update({"before_charge_preidction" : ScalarMLP})

    # calculate electrostatic energy for periodic or non-periodic systems
    # TODO: Ewald energy scaling

    if pbc:
        modules.update({"total_energy_with_qeq":EwaldQeq})
    else:
        modules.update({"total_energy_with_qeq":Qeq})

    modules.update({"one_hot2": ChargeEncoding})
    modules.update({"spharm_edges2": SphericalHarmonicEdgeAttrs})
    modules.update({"chemical_charge_embedding": ScalarMLP})
